...
def main():
    classfied = True
    classfied = False
    file2lst = read_file('/Users/gqs/Downloads/Antecedent_dataset_regression_analysis/random_dao.csv')
    if classfied:
...
def boxcoxt(input):
    transformed, lambda_value = boxcox(input)
    return transformed, lambda_value

def inverseboxcoxt(input, lambda_value):
    return inv_boxcox(input, lambda_value)
...
def MLR(regression_lst):
    ...
    independent_variables, lambda1 = boxcoxt(independent_variables)
    dependent_values, lambda2 = boxcoxt(dependent_values)
    ...
    y_pred = inverseboxcoxt(y_pred,lambda_value=lambda2)
    y_test = inverseboxcoxt(y_test,lambda_value=lambda2)
    y_train = inverseboxcoxt(y_train,lambda_value=lambda2)
    # y_pred = scalar.inverse_transform(y_pred)
    y_pred_train = xgb_model.predict(X_train_scaled)
    y_pred_train = inverseboxcoxt(y_pred_train,lambda_value=lambda2)
    ...
    def custom_mrse(y_true, y_pred):
        ...
        for i in range(len(y_true)):
            a = inverseboxcoxt(y_pred[i],lambda_value=lambda2)
            b = inverseboxcoxt(y_true[i],lambda_value=lambda2)
            sum += (((a / b) - 1) ** 2)
    ...
    y_test = inverseboxcoxt(y_test,lambda2)
    # 绘制散点图
