...
def MLR(regression_lst):
    ...
    xgb_model = SGDRegressor(**viral_sgd_params) #名称为xgb_model,内核改为skl的ridge模型
    ...
    # print(X_test.shape)
    # print(y_test.shape)
    # # Plot the fitted curve along with true data points
    # plt.figure(figsize=(10, 6))
    # plt.scatter(X_test[:,0], y_test, color='blue', alpha=0.7, label='True Data')
    # plt.plot(X_test, y_pred, color='red', linewidth=2, label='Fitted Curve')
    # plt.xscale('log')
    # plt.yscale('log')
    # plt.title('Fitted Curve vs. True Data (Log Scale)')
    # plt.xlabel('Log of Independent Variable')
    # plt.ylabel('Log of Dependent Variable')
    # plt.legend()
    # plt.show()
