...
class CNNAutoencoder(nn.Module):
    def __init__(self):
        ...
        # Decoder
        # Assuming the output of the encoder is of size [batch_size, 768, L], where L depends on the input size
        self.decoder = nn.Sequential(
            # Define the decoder layers that mirror the encoder but in reverse
            nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=(2,20), stride=(2,1), padding=0),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=(4,5), stride=(4,5), padding=0),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=(4,5), stride=(4,5), padding=0),
            nn.ReLU()
            )
